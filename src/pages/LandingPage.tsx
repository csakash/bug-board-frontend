...
function ProductMockup() {
  const cards = [
    { badge: 'Bug', badgeCls: 'text-rust', dot: '#c0552d', title: 'Upload fails on files over 10MB', meta: 'High · Open' },
    { badge: 'Feature', badgeCls: 'text-[#5b5bd6]', dot: '#d39a2b', title: 'Bulk-assign issues from the board', meta: 'Medium · In progress' },
    { badge: 'Improvement', badgeCls: 'text-[#3f6f4e]', dot: '#3f6f4e', title: 'Faster search across projects', meta: 'Low · Resolved' },
  ];
  return (
    <div className="animate-slide-up-soft rounded-xl border border-line bg-surface p-3 shadow-[0_20px_60px_-24px_rgba(43,41,37,0.35)] [animation-delay:120ms]">
      {/* window chrome */}
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-surface/80 backdrop-blur">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/login"
              className="premium-focus hidden rounded-md px-3 py-2 text-sm text-muted hover:text-ink sm:inline-block"
            >
              Log in
            </Link>
            <Link
              to="/login?mode=register"
              className="premium-focus rounded-md bg-rust px-3.5 py-2 text-sm font-medium text-white hover:bg-rust-dark active:scale-[0.99]"
            >
              Get started
            </Link>
          </div>
      ...
      <main>
        ...
        <section className="mx-auto grid max-w-6xl items-center gap-12 px-5 py-16 md:grid-cols-2 md:py-24">
          <div className="animate-fade-up">
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <Link
                to="/login?mode=register"
                className="premium-focus inline-flex items-center gap-1.5 rounded-md bg-rust px-4 py-2.5 text-sm font-medium text-white hover:bg-rust-dark active:scale-[0.99]"
              >
                Get started free
                <ArrowRight size={16} />
              </Link>
              <Link
                to="/login"
                className="premium-focus inline-flex items-center rounded-md border border-line bg-surface px-4 py-2.5 text-sm font-medium text-ink hover:border-rust hover:text-rust"
              >
                Log in
              </Link>
            </div>
        ...
        <section className="px-5 pb-20">
          <div className="mx-auto max-w-6xl overflow-hidden rounded-2xl border border-rust/20 bg-rust/[0.06] px-6 py-14 text-center md:py-16">
            ...
            <Link
              to="/login?mode=register"
              className="premium-focus mt-7 inline-flex items-center gap-1.5 rounded-md bg-rust px-5 py-3 text-sm font-medium text-white hover:bg-rust-dark active:scale-[0.99]"
            >
              Get started free
              <ArrowRight size={16} />
            </Link>
